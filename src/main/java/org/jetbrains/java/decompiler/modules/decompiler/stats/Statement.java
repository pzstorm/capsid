...
public class Statement implements IMatchable {
		...
		while (!stackNode.isEmpty())
		{
			...
			for (; index < lstEdges.size(); index++)
			{
				...
				if (!setVisited.contains(succ) &&
						(edge.getType() == StatEdge.TYPE_REGULAR || edge.getType() == StatEdge.TYPE_EXCEPTION))
				{ // TODO: edge filter?
	...
	private static void addToPostReversePostOrderList(Statement stat, List<? super Statement> lst, HashSet<?
			super Statement> setVisited) {
	...
	public void collapseNodesToStatement(Statement stat) {
		...
		if (post != null &&
				!stat.getNeighbours(StatEdge.TYPE_EXCEPTION, DIRECTION_FORWARD).contains(post))
	...
	private void addEdgeDirectInternal(int direction, StatEdge edge, int edgetype) {
		...
		mapStates.computeIfAbsent(edgetype, k -> new ArrayList<>()).add(direction == DIRECTION_BACKWARD ?
	...
	public IMatchable findObject(MatchNode matchNode, int index) {
		...
		if (node_type == MatchNode.MATCHNODE_STATEMENT && !this.stats.isEmpty())
		{
			...
			if (position != null)
			{
				...
				{
					return this.stats.get((this.stats.size() + Integer.parseInt(position)) % this.stats.size()); //
