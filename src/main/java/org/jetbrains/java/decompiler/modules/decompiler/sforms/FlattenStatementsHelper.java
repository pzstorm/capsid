...
public class FlattenStatementsHelper {
			...
			if (statEntry.succEdges == null)
			{
				...
				switch (stat.type)
				{
						...
						for (Statement st : stat.getStats())
						{
							...
							if (stat.type == Statement.TYPE_CATCHALL && ((CatchAllStatement) stat).isFinally())
							{
								...
								{ // catch head
									...
									stack.add(new StackEntry((CatchAllStatement) stat, Boolean.TRUE,
											StatEdge.TYPE_BREAK,
					...
					case Statement.TYPE_DO:
						...
						lstSuccEdges.add(stat.getSuccessorEdges(Statement.STATEDGE_DIRECT_ALL).get(0));  // exactly
						// one edge
						...
						switch (looptype)
						{
							...
							case DoStatement.LOOP_FOR:
								...
								DirectNode nodecond = new DirectNode(DirectNode.NODE_CONDITION, stat, stat.id +
										"_cond");
					...
					case Statement.TYPE_ROOT:
						...
						if (statementBreakIndex <= statsize)
						{
							...
							for (int i = statementBreakIndex; i < statsize; i++)
							{
								...
								lstStackStatements.addFirst(
										new StatementStackEntry(stat.getStats().get(i), stackFinally,
												(i == 0 && tailexprlst != null && tailexprlst.get(0) != null) ?
														tailexprlst : null));
							...
							if (stat.type == Statement.TYPE_IF && ((IfStatement) stat).iftype == IfStatement.IFTYPE_IF)
							{
								...
								// exactly one edge
								sourcenode = tailexprlst.get(0) == null ? node : graph.nodes.getWithKey(node.id +
										"_tail");
							}
						}
				}
			...
			if (sourcenode != null)
			{
				...
				for (int edgeindex = statEntry.edgeIndex; edgeindex < lstSuccEdges.size(); edgeindex++)
				{
					...
					while (true)
					{
						...
						if (entry == null)
						{
							saveEdge(sourcenode, destination, edgetype, isFinallyExit ? finallyShortRangeSource : null
							...
							{ // finally handler statement
								if (edgetype == StatEdge.TYPE_FINALLYEXIT)
								{
									...
									if (!catchall.containsStatementStrict(destination))
									{
										...
										saveEdge(sourcenode, destination, edgetype, isFinallyExit ?
														finallyShortRangeSource : null, finallyLongRangeSource,
								...
								if (!catchall.containsStatementStrict(destination))
								{
									...
									stack.add(new StackEntry(catchall, Boolean.TRUE, edgetype, destination,
											catchall.getHandler(),
											finallyLongRangeEntry == null ? catchall.getHandler() :
									...
									lstStackStatements.addFirst(statEntry);
									lstStackStatements.addFirst(new StatementStackEntry(catchall.getHandler(), stack,
											null));
	...
	private void setEdges() {
		...
		for (Edge edge : listEdges)
		{
			...
			DirectNode dest =
					graph.nodes.getWithKey(mapDestinationNodes.get(statid)[edge.edgetype == StatEdge.TYPE_CONTINUE ?
							1 : 0]);
		...
		for (int i = 0; i < 2; i++)
		{
					...
					mapLongRangeFinallyPathIds).entrySet())
			{
				...
				for (String[] arr : lst)
				{
					...
					DirectNode dest =
							graph.nodes.getWithKey(mapDestinationNodes.get(Integer.parseInt(arr[1]))[isContinueEdge ?
									1 : 0]);
