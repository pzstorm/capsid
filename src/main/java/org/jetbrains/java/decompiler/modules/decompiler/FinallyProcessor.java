...
public class FinallyProcessor {
		...
		for (BasicBlock block : setTry)
		{
			...
			for (BasicBlock dest : lstSucc)
			{
				...
				if (dest != graph.getLast() && !setCopy.contains(dest))
				{
					...
					seq.addInstruction(Instruction.create(CodeConstants.opc_bipush, false, CodeConstants.GROUP_GENERAL
							, bytecode_version, new int[]{ 0 }), -1);
					seq.addInstruction(Instruction.create(CodeConstants.opc_istore, false, CodeConstants.GROUP_GENERAL
							, bytecode_version, new int[]{ var }), -1);
	...
	private Area compareSubgraphsEx(ControlFlowGraph graph,
		...
		while (!stack.isEmpty())
		{
			...
			if (!compareBasicBlocksEx(graph, blockCatch, blockSample, (isFirstBlock ? 1 : 0) | (isTrueLastBlock ? 2 :
							0), finallytype,
					entry.lstStoreVars)) {
				return null;
			}
			...
			if (isLastBlock)
			{
				...
				for (BasicBlock succ : setSuccs)
				{
					...
					{ // FIXME: why?
						mapNext.put(blockSample.id + "#" + succ.id, new BasicBlock[]{ blockSample, succ,
								isTrueLastBlock ? succ : null });
					}
				}
			}
		}
