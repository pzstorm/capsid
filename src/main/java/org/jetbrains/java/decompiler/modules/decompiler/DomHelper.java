...
public final class DomHelper {
	...
	public static VBStyleCollection<List<Integer>, Integer> calcPostDominators(Statement container) {
		...
		List<Statement> lstStats =
				container.getPostReversePostOrderList(StrongConnectivityHelper.getExitReps(components));
			...
			{ // FIXME: implement proper node splitting. For now up to 5 nodes in sequence are splitted.
				...
				if (reducibility > 0)
				{
					...
					if (IrreducibleCFGDeobfuscator.isStatementIrreducible(general))
					{
						...
						{ // last chance lost
							DecompilerContext.getLogger().writeMessage("Statement cannot be decomposed although " +
									"reducible!", IFernflowerLogger.Severity.ERROR);
						}
						break;
					}
				...
				for (int i = 0; i < 2; i++)
				{
					...
					while (true)
					{
						...
						if (stat != null)
						{
							boolean complete = processStatement(stat, general.getFirst() == stat ? mapExtPost :
									new HashMap<>());
