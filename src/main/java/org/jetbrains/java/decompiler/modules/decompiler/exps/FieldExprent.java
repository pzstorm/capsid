...
public class FieldExprent extends Exprent {
	...
	public FieldExprent(LinkConstant cn, Exprent instance, Set<Integer> bytecodeOffsets) {
		this(cn.elementname, cn.classname, instance == null, instance, FieldDescriptor.parseDescriptor(cn.descriptor),
				bytecodeOffsets);
	}
	...
	public int getExprentUse() {
		return 0; // multiple references to a field considered dangerous in a multithreaded environment, thus no
		// Exprent.MULTIPLE_USES set here
	}
	...
	public TextBuffer toJava(int indent, BytecodeMappingTracer tracer) {
		...
		if (isStatic)
		{
			...
			if (super_qualifier != null) {
				...
				boolean casted = ExprProcessor.getCastedExprent(instance, new VarType(CodeConstants.TYPE_OBJECT, 0,
						classname), buff, indent, true, tracer);
			...
			if (buf.toString().equals(
					VarExprent.VAR_NAMELESS_ENCLOSURE))
