...
public class NewExprent extends Exprent {
			...
			if (!lstParams.isEmpty())
			{
				...
				if (enclosing.type == Exprent.EXPRENT_VAR)
				{
					...
					String this_classname =
							varEnclosing.getProcessor().getThisVars().get(new VarVersionPair(varEnclosing));
	...
	public VarType getExprType() {
		return anonymous ?
				DecompilerContext.getClassProcessor().getMapRootClasses().get(newType.value).anonymousClassType :
				newType;
	}
	...
	public TextBuffer toJava(int indent, BytecodeMappingTracer tracer) {
		...
		if (anonymous)
		{
				...
				if (selfReference) {
					...
					if (enclosing != null)
					{
						ClassNode anonymousNode =
								DecompilerContext.getClassProcessor().getMapRootClasses().get(child.anonymousClassType.value);
			...
			if (!lambda && constructor != null)
			{
				...
				List<VarVersionPair> mask = child.getWrapper().getMethodWrapper(CodeConstants.INIT_NAME,
						constructor.getStringDescriptor()).synthParameters;
				if (mask == null)
				{
					InvocationExprent superCall = child.superInvocation;
					mask = ExprUtil.getSyntheticParametersMask(superCall.getClassname(),
							superCall.getStringDescriptor(), parameters.size());
				}
				...
				for (int i = start; i < parameters.size(); i++)
				{
					if (mask == null || mask.get(i) == null)
					{
						...
						ExprProcessor.getCastedExprent(parameters.get(i), constructor.getDescriptor().params[i], buf,
								indent, true, tracer);
		...
		else if (newType.arrayDim == 0)
		{
			...
			if (constructor != null)
			{
				...
				List<VarVersionPair> mask = ExprUtil.getSyntheticParametersMask(constructor.getClassname(),
						constructor.getStringDescriptor(), parameters.size());
