	...
	public InvocationExprent(int opcode,
		...
		if (opcode == CodeConstants.opc_invokedynamic)
		{
			...
			if (dynamicInvocationType == CodeConstants.CONSTANT_MethodHandle_REF_invokeStatic) {
				...
				// FIXME: remove the first parameter completely from the list. It's the object type for a virtual
				//  lambda method.
	...
	public TextBuffer toJava(int indent, BytecodeMappingTracer tracer) {
		...
		if (isStatic)
		{
			if (isBoxingCall() && canIgnoreBoxing)
			{
				...
				ExprProcessor.getCastedExprent(lstParameters.get(0), descriptor.params[0], buf, indent, false, false,
						false, false, tracer);
				return buf;
			}
		...
		if (functype == TYP_INIT)
		{
			...
			if (newNode != null)
			{
				...
				isEnum =
						newNode.classStruct.hasModifier(CodeConstants.ACC_ENUM) && DecompilerContext.getOption(IFernflowerPreferences.DECOMPILE_ENUM);
			}
		}
		...
		for (int i = start; i < lstParameters.size(); i++)
		{
			if (mask == null || mask.get(i) == null)
			{
				...
				ExprProcessor.getCastedExprent(lstParameters.get(i), descriptor.params[i], buff, indent, true,
						ambiguous, true, true, tracer);
