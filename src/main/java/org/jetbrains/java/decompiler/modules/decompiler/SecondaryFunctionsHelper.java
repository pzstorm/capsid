...
public final class SecondaryFunctionsHelper {
	...
	static
	{
		...
		mapNumComparisons.put(FunctionExprent.FUNCTION_NE,
				new Integer[]{ FunctionExprent.FUNCTION_GE, FunctionExprent.FUNCTION_NE,
						FunctionExprent.FUNCTION_LE });
		mapNumComparisons.put(FunctionExprent.FUNCTION_GT, new Integer[]{ FunctionExprent.FUNCTION_GE,
				FunctionExprent.FUNCTION_GT, null });
		mapNumComparisons.put(FunctionExprent.FUNCTION_GE, new Integer[]{ null, FunctionExprent.FUNCTION_GE,
				FunctionExprent.FUNCTION_GT });
		mapNumComparisons.put(FunctionExprent.FUNCTION_LT, new Integer[]{ null, FunctionExprent.FUNCTION_LT,
				FunctionExprent.FUNCTION_LE });
		mapNumComparisons.put(FunctionExprent.FUNCTION_LE, new Integer[]{ FunctionExprent.FUNCTION_LT,
				FunctionExprent.FUNCTION_LE, null });
	}
	...
	public static boolean identifySecondaryFunctions(Statement stat, VarProcessor varProc) {
		if (stat.getExprents() == null)
		{
			...
			if (stat.type == Statement.TYPE_IF)
			{
						...
						ifstat.getExprents().isEmpty() && (ifstat.getAllSuccessorEdges().isEmpty() || !ifstat.getAllSuccessorEdges().get(0).explicit))
				{
					...
					ifelsestat.getHeadexprentList().set(0,
							((IfExprent) ifelsestat.getHeadexprent().copy()).negateIf());
		...
		while (replaced)
		{
			...
			List<Object> lstObjects = new ArrayList<>(stat.getExprents() == null ? stat.getSequentialObjects() :
					stat.getExprents());
	...
	private static Exprent identifySecondaryFunctions(Exprent exprent, boolean statement_level, VarProcessor varProc) {
					...
					if (expr1.type == Exprent.EXPRENT_FUNCTION && expr2.type == Exprent.EXPRENT_CONST)
					{
								...
								functype == FunctionExprent.FUNCTION_DCMPL)
						{
							...
							if (desttype >= 0)
							{
								...
								{
									boolean oneForNan =
											functype == FunctionExprent.FUNCTION_DCMPL || functype == FunctionExprent.FUNCTION_FCMPL;
									boolean trueForOne =
											desttype == FunctionExprent.FUNCTION_LT || desttype == FunctionExprent.FUNCTION_LE;
									...
									if (trueForNan)
									{
										...
										return new FunctionExprent(FunctionExprent.FUNCTION_BOOL_NOT, operands,
												funcexpr.bytecode);
									}
								}
		...
		switch (exprent.type)
		{
				...
				switch (fexpr.getFuncType())
				{
					...
					case FunctionExprent.FUNCTION_DCMPG:
						...
						FunctionExprent iff = new FunctionExprent(FunctionExprent.FUNCTION_IIF, Arrays.asList(
								new FunctionExprent(FunctionExprent.FUNCTION_LT, Arrays.asList(new VarExprent(var,
												type, varProc),
			...
			case Exprent.EXPRENT_INVOCATION:
				if (!statement_level)
				{ // simplify if exprent is a real expression. The opposite case is pretty absurd, can still happen
					// however (and happened at least once).
					...
					switch (ftype)
					{
						...
						case FunctionExprent.FUNCTION_LE:
							if (!canSimplify)
							{
								...
								{
									canSimplify =
											commonSupertype.type != CodeConstants.TYPE_FLOAT && commonSupertype.type != CodeConstants.TYPE_DOUBLE;
								}
							}
							if (canSimplify)
							{
