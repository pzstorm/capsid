		...
		if (st.type == Statement.TYPE_IF)
		{
					...
					ifbranch.getExprents().get(0).type == Exprent.EXPRENT_EXIT)
			{
				...
				if (second_param.type == Exprent.EXPRENT_CONST &&
						second_param.getExprType().type == CodeConstants.TYPE_NULL)
									...
									if (param_annotations_lists.size() >= method_param_number - i)
									{
										int shift = method_param_number -
												param_annotations_lists
														.size(); // NOTE: workaround for compiler bug, count
				...
				if (exit_exprent.getExitType() == ExitExprent.EXIT_RETURN)
				{
					...
					{ // TODO: reversed order possible (in theory)
						...
						if (second_param.type == Exprent.EXPRENT_CONST &&
								second_param.getExprType().type == CodeConstants.TYPE_NULL)
						{ // TODO: reversed parameter order
							//if(first_param.type == Exprent.EXPRENT_VAR && ((VarExprent)first_param).getIndex() ==
							// var_value.getIndex()) {
							...
							{ // TODO: reversed parameter order
								...
								{        // TODO: check for absence of side effects like method invocations etc.
											...
											ifbranch.getExprents().get(0).type == Exprent.EXPRENT_EXIT)
									{
										...
										ifstat.removeSuccessor(ifstat.getAllSuccessorEdges().get(0)); // remove 'else'
										// edge
