...
public final class IfHelper {
		...
		if (rtnode.edgetypes.get(1) == 0)
		{
					...
					if (secondif.getFirst().getExprents().isEmpty())
					{
						...
						if (path == 2) {
							lstOperands.set(0, new FunctionExprent(FunctionExprent.FUNCTION_BOOL_NOT,
									lstOperands.get(0), null));
						}

						lstOperands.add(statexpr.getCondition());

						statexpr
								.setCondition(new FunctionExprent(path == 1 ? FunctionExprent.FUNCTION_COR :
			...
			else if (elsebranch.succs.size() == 1)
			{
				if (elsebranch.succs.get(0).value == rtnode.succs.get(0).value)
				{
					...
					statexpr
							.setCondition(new FunctionExprent(FunctionExprent.FUNCTION_BOOL_NOT,
									statexpr.getCondition(), null));
	...
	private static boolean reorderIf(IfStatement ifstat) {
		...
		{  // if - then
			...
			statexpr.setCondition(new FunctionExprent(FunctionExprent.FUNCTION_BOOL_NOT, statexpr.getCondition(),
					null));
