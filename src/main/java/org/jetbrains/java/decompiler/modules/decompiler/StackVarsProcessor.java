...
public class StackVarsProcessor {
	...
	private static boolean getUsedVersions(SSAUConstructorSparseEx ssa, VarVersionPair var, List<?
			super VarVersionNode> res) {
	...
	private static boolean isVersionToBeReplaced(VarVersionPair usedvar,
		...
		for (Entry<Integer, Set<VarVersionPair>> ent : mapVars.entrySet())
		{
			...
			{
				VarVersionNode node = ssuversions.nodes.getWithKey(new VarVersionPair(ent.getKey().intValue(),
						livever.intValue()));
	...
	private int[] iterateExprent(List<Exprent> lstExprents,
		...
		// stack variables only
		if (!left.isStack() &&
				(right.type != Exprent.EXPRENT_VAR || ((VarExprent) right).isStack()))
