...
public class SimplifyExprentsHelper {
					...
					if (econst.type == Exprent.EXPRENT_CONST && ((ConstExprent) econst).hasValueOne())
					{
						...
						if (left.type != Exprent.EXPRENT_VAR && left.equals(econd))
						{
							int type = func.getFuncType() == FunctionExprent.FUNCTION_ADD ?
									FunctionExprent.FUNCTION_PPI : FunctionExprent.FUNCTION_MMI;
		...
		if (exprent.type == Exprent.EXPRENT_INVOCATION)
		{
			...
			if (in.getInvocationTyp() == InvocationExprent.INVOKE_DYNAMIC)
			{
				...
				ClassNode lambda_class =
						DecompilerContext.getClassProcessor().getMapRootClasses().get(lambda_class_name);
						...
						ifStatement.getAllSuccessorEdges().get(0).getDestination() == elseStatement.getAllSuccessorEdges().get(0).getDestination())
				{
						...
						if (ifAssign.getLeft().type == Exprent.EXPRENT_VAR && elseAssign.getLeft().type == Exprent.EXPRENT_VAR)
						{
							...
							{ // ifVar.getIndex() >= VarExprent.STACK_BASE) {
								...
								if (found)
								{
									...
									List<Exprent> operands = Arrays.asList(statement.getHeadexprent().getCondition(),
											ifAssign.getRight(), elseAssign.getRight());
									data.add(new AssignmentExprent(ifVar,
											new FunctionExprent(FunctionExprent.FUNCTION_IIF, operands,
													ifHeadExprBytecode), ifHeadExprBytecode));
									...
									if (statement.getAllSuccessorEdges().isEmpty())
									{
										...
										StatEdge edge = new StatEdge(ifEdge.getType(), statement,
					...
					else if (ifExpr.type == Exprent.EXPRENT_EXIT && elseExpr.type == Exprent.EXPRENT_EXIT)
					{
								...
								ifExit.getExitType() == ExitExprent.EXIT_RETURN)
						{
							...
							// e.g. throws IOException and throw true?new RuntimeException():new IOException(); won't
							// work
							if (ifExit.getExitType() == ExitExprent.EXIT_THROW &&
									!ifExit.getValue().getExprType().equals(elseExit.getValue().getExprType()))
							...
							statement.addSuccessor(new StatEdge(StatEdge.TYPE_BREAK, statement,
									retEdge.getDestination(), closure));
	...
	private static boolean collapseInlinedClass14(Statement stat) {
		...
		if (ret)
		{
			...
			assignment.replaceExprent(assignment.getRight(), new ConstExprent(VarType.VARTYPE_CLASS, class_name,
					null));
			...
			ClassWrapper wrapper =
					(ClassWrapper) DecompilerContext.getProperty(DecompilerContext.CURRENT_CLASS_WRAPPER);
			if (wrapper != null) {
	...
	public boolean simplifyStackVarsStatement(Statement stat, Set<Integer> setReorderedIfs, SSAConstructorSparseEx ssa
			, StructClass cl) {
		boolean res = false;
			...
			while (true)
			{
				...
				for (Statement st : stat.getStats())
				{
					...
					changed = IfHelper.mergeIfs(st, setReorderedIfs) ||  // collapse composed if's
							...
							processClass14 && collapseInlinedClass14(st);  // collapse inlined .class property in
