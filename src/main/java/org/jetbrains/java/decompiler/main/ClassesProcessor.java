...
public class ClassesProcessor implements CodeConstants {
	...
	private static boolean isAnonymous(StructClass cl, StructClass enclosingCl) {
		...
		if (interfaces.length > 0)
		{
			boolean hasNonTrivialSuperClass =
					cl.superClass != null && !VarType.VARTYPE_OBJECT.equals(new VarType(cl.superClass.getString(),
							true));
			if (hasNonTrivialSuperClass || interfaces.length > 1)
			{ // can't have multiple 'sources'
				String message = "Inconsistent anonymous class definition: '" + cl.qualifiedName + "'. Multiple " +
						"interfaces and/or super class defined.";
	...
	private static void addClassnameToImport(ClassNode node, ImportCollector imp) {
		if (node.simpleName != null && node.simpleName.length() > 0) {
			imp.getShortName(node.type == ClassNode.CLASS_ROOT ? node.classStruct.qualifiedName : node.simpleName,
					false);
		}
				...
				if (bDecompileInner)
				{
					...
					if (inner != null)
					{
						for (StructInnerClassesAttribute.Entry entry : inner.getEntries())
						{
							...
							rec.type = entry.simpleNameIdx == 0 ? ClassNode.CLASS_ANONYMOUS : entry.outerNameIdx == 0
									? ClassNode.CLASS_LOCAL : ClassNode.CLASS_MEMBER;
							...
							{ // own classes only
								...
								else if (!Inner.equal(existingRec, rec))
								{
									...
									DecompilerContext.getLogger().writeMessage(message,
				...
				if (!mapInnerClasses.containsKey(ent.getKey()))
				{
					...
					while (!stack.isEmpty())
					{
						...
						if (setNestedClasses != null)
						{
							...
							StructInnerClassesAttribute inner =
									scl.getAttribute(StructGeneralAttribute.ATTRIBUTE_INNER_CLASSES);

							if (inner == null || inner.getEntries().isEmpty())
							{
								DecompilerContext.getLogger().writeMessage(superClass + " does not contain inner " +
										"classes!", IFernflowerLogger.Severity.WARN);
								continue;
							}
							...
							for (StructInnerClassesAttribute.Entry entry : inner.getEntries())
							{
								...
								if (nestedNode == null)
								{
									DecompilerContext.getLogger().writeMessage("Nested class " + nestedClass + " " +
											"missing!", IFernflowerLogger.Severity.WARN);
									continue;
								}
