...
public class NestedClassProcessor {
			...
			for (Exprent expr : lst)
			{
				if (expr.type == Exprent.EXPRENT_NEW)
				{
					...
					VarNamesCollector enclosingCollector =
							new VarNamesCollector(enclosingMethod.varproc.getVarNames());
	...
	private static void computeLocalVarsAndDefinitions(ClassNode node) {
								...
								{ // non-static inner class constructor
									...
									if (nestedClassNode.type != ClassNode.CLASS_MEMBER)
									{
										List<VarFieldPair> mask =
												mapVarMasks.get(refClassName).get(constructor.getStringDescriptor());
										...
										for (int i = 0; i < mask.size(); i++)
										{
											...
											if (param.type == Exprent.EXPRENT_VAR && mask.get(i) != null)
											{
												...
												// FIXME: flags of variables are wrong! Correct the entire
												//  functionality.
												// if(method.varproc.getVarFinal(varPair) != VarTypeProcessor
												// .VAR_NON_FINAL) {
										...
										List<VarFieldPair> pairMask =
												mapVarFieldPairs.get(refClassName).get(constructor.getStringDescriptor());
										if (pairMask == null) {
											pairMask = lstTemp;
										}
										else {
										...
										mapVarFieldPairs.get(refClassName).put(constructor.getStringDescriptor(),
												pairMask);
		...
		for (Entry<String, Map<String, List<VarFieldPair>>> enclosing : mapVarMasks.entrySet())
		{
			...
			for (Entry<String, List<VarFieldPair>> entry : enclosing.getValue().entrySet())
			{
				...
				nestedNode.getWrapper().getMethodWrapper(CodeConstants.INIT_NAME, entry.getKey()).synthParameters =
						mask;
			}
		}
	}
					...
					for (VarVersionPair pair : method.synthParameters)
					{
						if (pair != null)
						{
							VarVersionPair newVar =
									new VarVersionPair(method.counter.getCounterAndIncrement(CounterContainer.VAR_COUNTER), 0);
					...
					for (Entry<String, VarVersionPair> entry : classNode.mapFieldsToVars.entrySet())
					{
						...
						if (classNode.type != ClassNode.CLASS_MEMBER)
						{
							MethodWrapper enclosing_method =
									classNode.parent.getWrapper().getMethods().getWithKey(classNode.enclosingMethod);
	...
	private static void setLocalClassDefinition(MethodWrapper method, ClassNode node) {
		...
		VarExprent var = new VarExprent(method.counter.getCounterAndIncrement(CounterContainer.VAR_COUNTER), classType
				, method.varproc);
	...
	public void processClass(ClassNode root, ClassNode node) {
		...
		if (node.type == ClassNode.CLASS_LAMBDA && !node.lambdaInformation.is_method_reference)
		{
			ClassNode node_content =
					DecompilerContext.getClassProcessor().getMapRootClasses().get(node.classStruct.qualifiedName);
