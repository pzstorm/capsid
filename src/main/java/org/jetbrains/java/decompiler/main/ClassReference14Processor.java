...
public final class ClassReference14Processor {
	...
	static
	{
		...
		NewExprent newExpr = new NewExprent(new VarType(CodeConstants.TYPE_OBJECT, 0,
				"java/lang/NoClassDefFoundError"), new ArrayList<>(), null);
		...
		invCause.setLstParameters(
				Collections.singletonList(new VarExprent(2, new VarType(CodeConstants.TYPE_OBJECT, 0, "java/lang" +
	...
	private static void processClassRec(ClassNode node, Map<ClassWrapper, MethodWrapper> mapClassMeths, Set<?
			super ClassWrapper> setFound) {
					...
					mt.hasModifier(CodeConstants.ACC_STATIC))
			{
				...
				if (root != null && root.getFirst().type == Statement.TYPE_TRYCATCH)
				{
					...
					if (cst.getStats().size() == 2 && cst.getFirst().type == Statement.TYPE_BASICBLOCK &&
							...
							cst.getVars().get(0).getVarType().equals(new VarType(CodeConstants.TYPE_OBJECT, 0, "java" +
				...
				if (fexpr.getLstOperands().get(0).type == Exprent.EXPRENT_FUNCTION)
				{
					...
					if (headexpr.getFuncType() == FunctionExprent.FUNCTION_EQ)
					{
								...
								((ConstExprent) headexpr.getLstOperands().get(1)).getConstType().equals(VarType.VARTYPE_NULL))
						{
							...
							ClassNode fieldnode =
									DecompilerContext.getClassProcessor().getMapRootClasses().get(field.getClassname());
